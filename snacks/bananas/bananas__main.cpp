#include "../../sauce/strange/strange.h"
#include "../../sauce/strange/implementation/baggage/strange__implementation__baggage.h"
#include "bananas__space.h"

#include <iostream>
#include <string>

struct banana
{
    auto eat(int xxx) -> void
    {
        std::cout << "banana.eat()\n";
    }

    auto ripen() -> void
    {
        std::cout << "banana.ripen()\n";
        _ripe = true;
    }

    auto ripe() const -> bool
    {
        std::cout << "banana.ripe():" << (_ripe ? "yes" : "no") << "\n";
        return _ripe;
    }

    auto peel() -> void
    {
        std::cout << "banana.peel()\n";
        _peeled = true;
    }

    auto peeled() const -> bool
    {
        std::cout << "banana.peeled():" << (_peeled ? "yes" : "no") << "\n";
        return _peeled;
    }

    auto pack(strange::bag & dest) const -> void
    {
        dest.from_object();
        dest.insert_object("ripe", dest.make_bool(_ripe));
        dest.insert_object("peeled", dest.make_bool(_peeled));
    }

    auto unpack(strange::bag const & src) -> void
    {
        src.get_object("ripe").as_bool(_ripe);
        src.get_object("peeled").as_bool(_peeled);
    }

private:
    bool _ripe = false;
    bool _peeled = false;
};

struct no_banana
{
    no_banana(std::string x)
    {
    }

    no_banana(no_banana const &) = delete;

    auto eat(int xxx) -> void
    {
        std::cout << "banana.eat()\n";
    }

    auto ripen() -> void
    {
        std::cout << "banana.ripen()\n";
        _ripe = true;
    }

    auto ripe() const -> bool
    {
        std::cout << "banana.ripe():" << (_ripe ? "yes" : "no") << "\n";
        return _ripe;
    }

    auto peel() -> void
    {
        std::cout << "banana.peel()\n";
        _peeled = true;
    }

    auto peeled() const -> bool
    {
        std::cout << "banana.peeled():" << (_peeled ? "yes" : "no") << "\n";
        return _peeled;
    }

    auto pack(strange::bag & dest) const -> void
    {
        dest.from_object();
        dest.insert_object("ripe", dest.make_bool(_ripe));
        dest.insert_object("peeled", dest.make_bool(_peeled));
    }

    auto unpack(strange::bag const & src) -> void
    {
        src.get_object("ripe").as_bool(_ripe);
        src.get_object("peeled").as_bool(_peeled);
    }

private:
    bool _ripe = false;
    bool _peeled = false;
};

struct bunch_of_bananas : std::vector<bananas::fruit>
{
    auto eat(int xxx) -> void
    {
        std::cout << "bunch_of_bananas.eat()\n";
    }

    auto pick() -> void
    {
        std::cout << "bunch_of_bananas.pick()\n";
        _picked = true;
    }

    auto picked() const -> bool
    {
        std::cout << "bunch_of_bananas.picked():" << (_picked ? "yes" : "no") << "\n";
        return _picked;
    }

    auto pack(strange::bag & dest) const -> void
    {
        dest.from_object();
        {
            auto _array = dest.make_array();
            for (auto const & _item : *this)
            {
                _array.push_back_array(dest.make_any(_item._static<strange::any>()));
            }
            dest.insert_object("fruit", _array);
        }
        dest.insert_object("picked", dest.make_bool(_picked));
    }

    auto unpack(strange::bag const & src) -> void
    {
        {
            auto _array = src.get_object("fruit").to_array();
            auto _size = _array.size();
            clear();
            resize(_size);
            for (std::size_t _index = 0; _index < _size; ++_index)
            {
                (*this)[_index] = _array[_index].to_any()._static<bananas::fruit>();
            }
        }
        src.get_object("picked").as_bool(_picked);
    }

private:
    bool _picked = false;
};

namespace strange
{

template<>
struct reflection<banana>
{
    static inline auto name() -> std::string
    {
        return "banana";
    }
};

template<>
struct reflection<no_banana>
{
    static inline auto name() -> std::string
    {
        return "no_banana";
    }
};

template<>
struct reflection<bunch_of_bananas>
{
    static inline auto name() -> std::string
    {
        return "bunch_of_bananas";
    }
};

}

int main()
{
    std::cout << "demo\n";

    auto fruit_1 = bananas::fruit::_make<banana>();
    auto fruit_2 = fruit_1;
    fruit_1.ripe();
    fruit_1.peeled();
    fruit_1.peel();
    fruit_1.peeled();
    fruit_1.eat();
    auto closure_1 = fruit_1.eat_closure_();
    closure_1(0);
    auto closure_2 = fruit_2.ripe_closure_();
    closure_2();

    auto bunch_1 = bananas::bunch<bananas::fruit, int64_t>::_make<std::vector<bananas::fruit>>();
    std::cout << "bunch_1._cat(): " << bunch_1._cat() << "\n";
    std::cout << "bunch_1._name(): " << bunch_1._name() << "\n";
    auto var0 = bananas::variadic<>::_make<std::vector<bananas::fruit>>();
    std::cout << "var0._cat(): " << var0._cat() << "\n";
    std::cout << "var0._name(): " << var0._name() << "\n";
    bunch_1.push_back(fruit_1);
    std::cout << "bunch_1.size(): " << bunch_1.size() << "\n";
    auto closure_3 = bunch_1.push_back_closure_();
    closure_3(fruit_2);
    std::cout << "bunch_1.size(): " << bunch_1.size() << "\n";

    auto bunch_2 = bananas::bunch_of_fruit::_make<bunch_of_bananas>();
    bunch_2.push_back(fruit_1);
    bunch_2.push_back(fruit_2);
    std::cout << "bunch_2.size(): " << bunch_2.size() << "\n";
    auto closure_4 = bunch_2.operator_array_closure_();
    std::cout << "closure_4(0).ripe()\n";
    closure_4(0).ripe();

    auto bag_1 = strange::baggage::_make();
    bag_1.from_any(bunch_2._static<strange::any>());
    std::cout << "bag_1.to_json(): " << bag_1.to_json() << "\n";
    auto bunch_3 = bag_1.to_any()._static<bananas::bunch_of_fruit>();
    std::cout << "bunch_3.size(): " << bunch_3.size() << "\n";

    std::cout << "fruit_1._name(): " << fruit_1._name() << "\n";
    auto fruit_3 = bananas::fruit::_manufacture("bananas::fruit_<banana, true>");
    std::cout << "fruit_3._valid(): " << fruit_3._valid() << "\n";
    std::cout << "fruit_3._name(): " << fruit_3._name() << "\n";

    auto no_banana_1 = bananas::fruit::_make<no_banana>(std::string{});
    std::cout << "no_banana_1._name(): " << no_banana_1._name() << "\n";

    auto fun_1 = bananas::functor<int32_t, int64_t>::_make<std::function<int32_t(int64_t)>>();
    std::cout << "fun_1._cat(): " << fun_1._cat() << "\n";
    auto fun_2 = bananas::functor<>::_make<std::function<void()>>();
    std::cout << "fun_2._name(): " << fun_2._name() << "\n";

    auto fung = bananas::fungible<>::_make();
    fung();
    std::cout << "fung._namme(): " << fung._name() << "\n";
    auto fung2 = bananas::fungible2<>::_make();
    fung2();
    fung2.fung();
    std::cout << "fung2._namme(): " << fung2._name() << "\n";

    return 0;
}
