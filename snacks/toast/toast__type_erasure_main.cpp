...
//#include "toast__type_erasure.h"
#include "toast__space_transformation_3.h"
...
struct implement
{
    ...
    inline void display(toast::button b) const
    {
...
void increment(toast::number & num)
{
    num.inc();
}
...
int main()
{
    ...
    auto w1 = toast::widget::_make<implement>();
    w1.inc();
    auto w2 = ++w1;
    w1.display(toast::button());
    w2.display(toast::button());
    w2.inc();
    w1.display(toast::button());
    w2.display(toast::button());

    auto b1 = toast::button::_make<implement>()--;
    if (b1._something())
    {
    ...
    if (w1._static<toast::number>()._valid())
    {
        std::cout << "yes" << b1._error() << std::endl;
    }
    else
    ...
    if (w1._dynamic<toast::number>()._something())
    {
        std::cout << "yes" << b1._error() << std::endl;
    }
    else
    ...
    auto n1 = toast::number::_make<implement>();
    n1.inc();
    n1.dec();

    auto wn1 = toast::widget_number::_make<implement>();
    wn1.inc();
    wn1.display(toast::button());

    toast::number wn2 = wn1;
    wn2.inc();

    toast::widget wn3 = wn1;
    wn3.inc();
    wn3.display(toast::button());
    ...
    auto ni1 = toast::numeric<>::_make<implement>();
    ni1.inc();
    auto ni2 = ni1;
    ni2.x() = 2;
    std::cout << ni1.get() << ni1.x() << ni2.x() << std::endl;

    auto wt = toast::widget::_make<implement_template<int>>();
    auto bt = toast::button::_make<implement_template<int>>()--;
    auto nt = toast::number::_make<implement_template<int>>();
    auto wnt = toast::widget_number::_make<implement_template<int>>();
    auto nit = toast::numeric<>::_make<implement_template<int>>();
    ...
    std::ifstream ifs{"toast__parser_input.h", std::ios::binary};
    if (!ifs.is_open())
    {
        std::cerr << "failed to open file: toast__parser_input.h\n";
        return 1;
    }
